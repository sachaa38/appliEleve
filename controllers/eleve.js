...
exports.addNote = async (req, res) => {
  ...
  try {
    ...
    if (!eleve) return res.status(404).json({ message: 'Elève non trouvé' })

    const newNote = { title, text, date }
    eleve.note.push(newNote)
...
exports.deleteNote = async (req, res) => {
  ...
  try {
    ...
    const note = eleve.note.id(req.params.noteId)
    if (!note) return res.status(404).json({ message: 'Note non trouvé' })

    eleve.note.pull(req.params.noteId)
...
exports.getAllEleve = async (req, res) => {
  try {
    const eleves = await Eleve.find({ userId: req.auth.userId })
